...
function initializeDatabase() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS blood_inventory (
      ...
      bank_id           INTEGER NOT NULL REFERENCES blood_banks(bank_id),
      blood_group_id    INTEGER NOT NULL REFERENCES blood_groups(blood_group_id),
      units_available   REAL NOT NULL DEFAULT 0,
      units_reserved    REAL NOT NULL DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS transfusions (
      ...
      transfusion_date  TEXT NOT NULL DEFAULT (datetime('now')),
      units_transfused  REAL NOT NULL,
      staff_id          INTEGER REFERENCES staff(staff_id),
      outcome           TEXT,
      notes             TEXT
    );
    CREATE INDEX IF NOT EXISTS idx_donors_bg   ON donors(blood_group_id);
    CREATE INDEX IF NOT EXISTS idx_donations_d ON donations(donor_id);
    CREATE INDEX IF NOT EXISTS idx_inv_bg      ON blood_inventory(bank_id, blood_group_id);
    CREATE INDEX IF NOT EXISTS idx_req_status  ON blood_requests(status);
  `);

  const count = db.prepare('SELECT COUNT(*) AS c FROM blood_groups').get().c;
...
function seedDatabase() {
  ...
  const insertBG = db.prepare(
    'INSERT OR IGNORE INTO blood_groups (group_name, rh_factor, abo_type, description) VALUES (?,?,?,?)');
  [['A+','+','A','Most common after O+'],['A-','-','A','Universal plasma donor'],
   ['B+','+','B','Second most common'], ['B-','-','B','Rare type'],
   ['O+','+','O','Universal RBC donor for + types'],['O-','-','O','Universal emergency donor'],
   ['AB+','+','AB','Universal recipient'],['AB-','-','AB','Rarest type']
  ].forEach(g => insertBG.run(...g));

  // Blood banks
  const ib = db.prepare('INSERT OR IGNORE INTO blood_banks (name,address,city,phone,email,license_no,established_date) VALUES (?,?,?,?,?,?,?)');
  ib.run('NUTech National Blood Centre','Sector I-12, Main IJP Road','Islamabad','0519283456','info@nutechblood.org','BB-ISB-001','2018-03-15');
  ib.run('Punjab Blood Service Lahore','Mall Road, Near Mayo Hospital','Lahore','0429871234','pbs@lahore.gov.pk','BB-LHR-002','2010-06-01');
  ib.run('Karachi Voluntary Blood Service','M.A. Jinnah Road, Saddar','Karachi','02135614416','kvbs@karachi.net','BB-KHI-003','2005-09-20');
  ib.run('Islamabad Safe Blood Centre','Blue Area, Jinnah Avenue','Islamabad','0519912233','isbc@pha.gov.pk','BB-ISB-004','2015-01-10');
  ib.run('Peshawar Blood Foundation','Cantonment, GT Road','Peshawar','0919256789','pbf@peshawar.org','BB-PEW-005','2012-07-22');

  // Hospitals
  const ih = db.prepare('INSERT OR IGNORE INTO hospitals (name,address,city,phone,email,license_no,hospital_type) VALUES (?,?,?,?,?,?,?)');
  [
    ...
    ['Shifa International','H-8/4, Islamabad','Islamabad','0519032000','info@shifa.com.pk','HS-ISB-005','General'],
    ...
    ['Jinnah Postgraduate','Rafiqui H.J. Shaheed Rd, Karachi','Karachi','02199201300','admin@jpmc.edu.pk','HS-KHI-009','Teaching'],
    ['Indus Hospital Karachi','Korangi Crossing, Karachi','Karachi','02135112709','info@indus.edu.pk','HS-KHI-010','General'],
  ].forEach(h => ih.run(...h));

  // Staff
  const is_ = db.prepare('INSERT OR IGNORE INTO staff (first_name,last_name,role,phone,email,bank_id,hire_date) VALUES (?,?,?,?,?,?,?)');
  [
  ...
  // Donors
  const id_ = db.prepare(`
    INSERT OR IGNORE INTO donors
    (first_name,last_name,dob,gender,phone,email,address,city,blood_group_id,
     registration_date,last_donation_date,total_donations,weight_kg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`);
  [
  ...
  const ip = db.prepare(`
    ...
    (first_name,last_name,dob,gender,phone,address,blood_group_id,hospital_id,medical_record_no,diagnosis)
    VALUES (?,?,?,?,?,?,?,?,?,?)`);
  [
  ...
  const inv = db.prepare(`
    ...
    (bank_id,blood_group_id,units_available,units_reserved,component_type,collection_date,expiry_date)
    VALUES (?,?,?,?,?,?,?)`);
  [
  ...
  const ir = db.prepare(`
    ...
    (patient_id,hospital_id,blood_group_id,units_requested,request_date,urgency_level,status,notes,approved_by,approved_date)
    VALUES (?,?,?,?,?,?,?,?,?,?)`);
  [
    ...
    [15,1,5,4.0,'2026-04-14 23:45:00','critical','fulfilled','Spleen rupture — massive transfusion',1,'2026-04-14 23:50:00'],
  ...
  // Donations (sample)
  const idon = db.prepare(`
    INSERT OR IGNORE INTO donations
    (donor_id,bank_id,staff_id,blood_group_id,donation_date,units_donated,hemoglobin_level,blood_pressure,status)
    VALUES (?,?,?,?,?,?,?,?,?)`);
  [
    [1,1,3,5,'2025-12-01 09:30:00',1.0,13.2,'118/76','completed'],
    [2,1,3,1,'2025-11-15 10:15:00',1.0,14.8,'122/80','completed'],
    [3,2,4,3,'2025-10-20 11:00:00',1.0,12.9,'115/75','completed'],
    [4,3,5,6,'2025-09-10 08:45:00',1.0,15.1,'120/78','completed'],
    [5,2,4,2,'2025-08-25 14:30:00',1.0,13.5,'112/72','completed'],
    [6,5,7,7,'2025-07-14 09:00:00',1.0,14.2,'124/82','completed'],
    [7,3,5,4,'2025-11-01 10:45:00',1.0,13.8,'118/76','completed'],
    [8,1,3,5,'2025-10-05 11:30:00',1.0,14.0,'120/80','completed'],
    [9,2,4,8,'2025-09-30 09:15:00',1.0,12.7,'110/70','completed'],
    [10,2,4,1,'2025-11-20 10:00:00',1.0,15.6,'126/84','completed'],
    [11,1,3,3,'2025-08-10 13:45:00',1.0,13.1,'116/74','completed'],
    [12,2,4,6,'2025-10-18 08:30:00',1.0,14.5,'122/80','completed'],
    [13,3,5,5,'2025-11-05 11:00:00',1.0,13.6,'119/77','completed'],
    [14,3,5,2,'2025-07-28 09:30:00',1.0,14.9,'124/82','completed'],
    [15,4,6,4,'2025-10-01 10:15:00',1.0,13.3,'115/75','pending'],
    [16,5,7,7,'2025-09-15 08:00:00',1.0,15.0,'128/84','completed'],
    [17,3,5,1,'2025-08-22 14:00:00',1.0,13.0,'113/73','completed'],
    [18,1,3,3,'2025-11-10 11:45:00',1.0,14.3,'120/78','completed'],
    [19,2,4,6,'2025-10-25 09:00:00',1.0,13.7,'117/75','completed'],
    [20,1,3,5,'2025-11-28 10:30:00',1.0,15.4,'122/80','completed'],
  ].forEach(d => idon.run(...d));

  console.log('[DB] Database seeded with 20 records per table.');
}
